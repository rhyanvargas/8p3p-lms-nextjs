...
export function ChapterQuiz({ quiz, onNextChapter, chapterTitle, chapterId }: ChapterQuizProps) {
  ...
  const handleSubmit = () => {
    ...
    const correctAnswers = selectedOptions.reduce((count, selected, index) => {
      return selected === quiz.questions[index].correctOption ? count + 1 : count;
    }, 0);
        ...
        <CardContent className="p-6">
          <div className="text-center mb-6">
            ...
            <p className="text-muted-foreground">
              You answered {selectedOptions.filter((option, index) => option === quiz.questions[index].correctOption).length} out of {quiz.questions.length} questions correctly.
            </p>
          </div>
                ...
                <div key={question.id} className={`p-4 rounded-lg ${isCorrect ? 'bg-green-50' : 'bg-red-50'}`}>
                  <div className="flex items-start gap-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm">
                        <span className="font-medium">Your answer:</span> {question.options[selectedOptions[index]]}
                      </p>
                      {!isCorrect && (
                        <p className="text-sm text-green-600">
                          <span className="font-medium">Correct answer:</span> {question.options[question.correctOption]}
                        </p>
                      )}
                    </div>
                  </div>
      ...
      <CardContent className="p-6">
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            ...
            <span className="text-sm text-muted-foreground">{Math.round(((currentQuestion + 1) / quiz.questions.length) * 100)}% complete</span>
          </div>
        ...
        <div className="mb-6">
          ...
          <RadioGroup 
            ...
            onValueChange={(value: string) => handleOptionSelect(parseInt(value))}
          >
